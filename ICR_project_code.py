...
- Computes expected runs using comparison group's avg runs per ball * (avg balls faced by group if out, else player's balls)
- Volume adjustment as sum of per-venue avg(adjusted runs / innings) using unique innings (p_match, inns)
- Derives balls from ball_id if balls column is missing
- Aggregates out column with max to capture dismissal status
- Prints step-by-step detailed calculation for a chosen player with out/not-out summary
"""
...
# Sort the DataFrame to ensure sequential order
df = df.sort_values(['p_match', 'inns', 'ball_id'])

# Compute previous wprob within each innings
df['prev_wprob'] = df.groupby(['p_match', 'inns'])['wprob'].shift(1)

# Compute delta_wprob, filling NaN (first ball) with 0
df['delta_wprob'] = (df['wprob'] - df['prev_wprob']).fillna(0)

# Restrict to balls where final_batting_position <= 8 for the batter
df_top8 = df[df['final_batting_position'] <= 9].copy()

# Compute WPA per match per batter
wpa_per_match = df_top8.groupby(['p_match', 'bat'])['delta_wprob'].sum().reset_index(name='wpa')

# Get final_batting_position per match per batter
position_per_match = df_top8.groupby(['p_match', 'bat'])['final_batting_position'].first().reset_index(name='position')

# Merge WPA with positions
wpa_per_match = wpa_per_match.merge(position_per_match, on=['p_match', 'bat'])

# Compute total runs per match per batter
runs_per_match = df_top8.groupby(['p_match', 'bat'])['score'].sum().reset_index(name='runs')
wpa_per_match = wpa_per_match.merge(runs_per_match, on=['p_match', 'bat'])

# Filter players with at least 3 matches
match_count = df_top8.groupby('bat')['p_match'].nunique().reset_index(name='match_count')
valid_players = match_count[match_count['match_count'] >= 3]['bat']
wpa_per_match = wpa_per_match[wpa_per_match['bat'].isin(valid_players)]

# Define grouping logic for batting positions
def get_group(pos):
    if pos in [1, 2]:
        return 'Openers'
    elif pos in [3, 4]:
...
wpa_per_match['group'] = wpa_per_match['position'].apply(get_group)

# Compute total WPA, matches, and runs per player per group
player_group_stats = wpa_per_match.groupby(['bat', 'group']).agg(
    total_wpa=('wpa', 'sum'),
    num_matches=('p_match', 'nunique'),
    total_runs=('runs', 'sum')
).reset_index()

# Filter players with at least 100 runs at that position
player_group_stats = player_group_stats[player_group_stats['total_runs'] >= 130]

# Compute average WPA per match
player_group_stats['avg_wpa'] = player_group_stats['total_wpa'] / player_group_stats['num_matches']

# Function to calculate percentile within each group
def calculate_percentile(group):
    group['percentile'] = group['avg_wpa'].rank(pct=True) * 100
    return group

# Apply percentile calculation per group
player_group_stats = player_group_stats.groupby('group').apply(calculate_percentile).reset_index(drop=True)

# Sort within each group by percentile descending
player_group_stats = player_group_stats.sort_values(['group', 'percentile'], ascending=[True, False])

# Display the results
print(player_group_stats)
...
# params - tune k if you like
K = 5.0
group_col = 'group'
player_col = 'bat'
n_col = 'match_count'

# assume player_stats is already loaded as a DataFrame
df = player_stats.copy()

# 1) rename original columns to *_initial (safe: create cols if missing)
if 'ICR' in df.columns:
    df = df.rename(columns={'ICR': 'ICR_initial'})
else:
    df['ICR_initial'] = np.nan

if 'ICR_percentile' in df.columns:
    df = df.rename(columns={'ICR_percentile': 'ICR_percentile_initial'})
else:
    df['ICR_percentile_initial'] = np.nan

# ensure numeric types for ICR_initial and match_count
df['ICR_initial'] = pd.to_numeric(df['ICR_initial'], errors='coerce')
df[n_col] = pd.to_numeric(df.get(n_col, 0), errors='coerce').fillna(0).astype(float)

# 2) compute group means (mu) using available ICR_initial values
group_mu = df.groupby(group_col)['ICR_initial'].mean().rename('mu_group')
df = df.merge(group_mu, left_on=group_col, right_index=True, how='left')

# fallback global mean if a group mean is NaN
global_mu = df['ICR_initial'].mean(skipna=True)

# 3) apply shrinkage: alpha = n / (n + k)
def shrink_icr(row, k=K, global_mu_val=global_mu):
    x = row['ICR_initial']
    mu = row['mu_group']
    n = row[n_col]
    if pd.isna(mu):
        mu = global_mu_val
    # if player's x is NaN, preserve NaN (do not impute)
    if pd.isna(x):
        return np.nan
    if (n is None) or (n <= 0) or pd.isna(n):
        alpha = 0.0
...
df['ICR'] = df.apply(shrink_icr, axis=1)

# 4) compute new percentile from ICR within each group (0..100)
def compute_percentile_groupwise(sub):
    s = sub['ICR'].astype(float)
    non_na = s.dropna()
    n = len(non_na)
    if n == 0:
        return pd.Series([np.nan] * len(s), index=sub.index)
    # ranks among non-NaN; using average rank for ties
    ranks = s.rank(method='average', na_option='keep')
    if n == 1:
        pct = ranks.where(ranks.isna(), 100.0)
    else:
...
df['ICR_percentile'] = df.groupby(group_col, group_keys=False).apply(compute_percentile_groupwise)

# 5) cleanup helper column
df = df.drop(columns=['mu_group'])

# 6) quick diagnostics / sample
print(f"Applied shrinkage with K = {K}. Sample rows:")
display_cols = [player_col, group_col, n_col, 'ICR_initial', 'ICR', 'ICR_percentile_initial', 'ICR_percentile']
...
player_group_stats=player_group_stats[player_group_stats['num_matches']>=5]

results_df['bat']=results_df['player']
...
# ---------------- CONSTANTS ----------------
DEFAULT_PRIOR = {'rvap': 0.33, 'icr': 0.33, 'wpa': 0.34}
DEFAULT_ALPHA = 0.5
MIN_MATCHES_DEFAULT = 5
WINSOR_PCT = 0.01
DEFAULT_MIN_BALLS_THRESHOLD = 70
MIN_MATCHES_FALLBACK = 3
MIN_GROUP_SIZE = 10

# ---------------- HELPER FUNCTIONS ----------------

def clean_phase_groups(df, col="phase_groups"):
    """
    Ensure phase_groups is always a clean string:
    - If list/array -> join with '+'
    - If NaN -> 'Other'
    - If string -> keep
    """
    if col not in df.columns:
    ...
    def normalize(val):
        # ✅ Corrected logic: Check for list/array types *before* checking for NaN.
    ...
    # Use a try-except block to handle potential type errors gracefully,
    # although the corrected logic should prevent the specific ValueError.
    try:
        df[col] = df[col].apply(normalize)
    except Exception as e:
        print(f"Error applying normalize to column '{col}': {e}")
        return df

    return df

def _extract_primary_group(phase_groups_str):
    """
    Extracts the primary phase group from a '+' separated string.
    Assumes first part is the most important.
    """
    if isinstance(phase_groups_str, str) and '+' in phase_groups_str:
        return phase_groups_str.split('+')[0]
    return phase_groups_str
def _winsorize_series(series, pct):
    if series.empty:
        return series
    q_low, q_high = series.quantile([pct, 1 - pct]).values
    return series.clip(lower=q_low, upper=q_high)

def _standardize_series(series):
    """Standardizes a series (mean 0, std 1), handling zero standard deviation."""
    series = series.astype(float)
    mean_val = series.mean()
    std_val = series.std(ddof=1)

    if std_val == 0 or np.isnan(std_val):
        return (series - mean_val).fillna(0) # Returns a series of zeros if no variance
    else:
        return (series - mean_val) / std_val

def compute_weights_reliability(n1, n2, n3):
    """Computes weights based on the reliability (number of observations)."""
    n_sum = n1 + n2 + n3
    n_sum[n_sum == 0] = 1 # Avoid division by zero
    w1 = n1 / n_sum
    w2 = n2 / n_sum
    w3 = n3 / n_sum
    return w1, w2, w3

def compute_weights_invvar(v1, v2, v3, n1_avg, n2_avg, n3_avg):
    """Computes weights based on inverse variance, with a stability constant."""
    # Use average n values as a proxy for stability constant if real data not available
    stable_const1 = n1_avg / 100.0 if n1_avg > 0 else 1.0
    stable_const2 = n2_avg / 100.0 if n2_avg > 0 else 1.0
    stable_const3 = n3_avg / 100.0 if n3_avg > 0 else 1.0

    std1 = np.std(v1) + stable_const1
    std2 = np.std(v2) + stable_const2
    std3 = np.std(v3) + stable_const3

    inv_var_sum = 1/std1 + 1/std2 + 1/std3
    if inv_var_sum == 0:
        return [1/3, 1/3, 1/3]
...
def compute_weights_pca(v1, v2, v3):
    """Computes weights based on Principal Component Analysis."""
    ...
    comp = pca.components_[0]
    weights = np.abs(comp) / np.sum(np.abs(comp))
    return list(weights), pca.explained_variance_ratio_[0]

def compute_weights_regression(v1, v2, v3, outcome):
    """Computes weights using a linear regression model."""
    data = np.vstack([v1, v2, v3]).T
    # Align data and outcome, dropping NaNs
    valid_mask = ~np.isnan(data).any(axis=1) & ~np.isnan(outcome)
    ...
    r2 = model.score(data, outcome)

    return list(weights), r2

# ---------------- MASTER COMBINE FUNCTION ----------------
...
# NOTE: This is a demonstration. Replace this with your actual DataFrame objects.
# The sample data is constructed to test the function's logic and the new helper functions.
print("Creating mock dataframes for demonstration...")

# Results Da
# results_df = pd.DataFrame(data_results)


# player_group_stats = pd.DataFrame(data_player_group_stats)

# Now, the function call will work without error.
try:
    merged, weights = combine_bowler_scores_phasegroups_master(
        ...
        winsor_pct=0.01, consistency_coefficient=3.0, min_balls_threshold=40
    )
    print("\n--- Execution Successful ---")
    print("\nResulting DataFrame Head:")
    print(merged.head())
    print("\nWeight Summary:")
    for group, summary in weights.items():
        if group != 'validation':
            print(f"Group: {group}")
            print(f"  Final Weights: {summary['final_weights']}")
    print("\nValidation Summary:")
    print(weights.get('validation'))
